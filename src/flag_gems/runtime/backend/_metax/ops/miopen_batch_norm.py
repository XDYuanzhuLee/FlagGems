...
def batch_norm_forward_kernel(
    ...
    if is_train:
        ...
        running_mean_ptr = running_mean_pointer + feat_pid
        running_var_ptr = running_var_pointer + feat_pid
...
class BatchNorm(torch.autograd.Function):
    ...
    def forward(
        ...
        logger.debug("METAX GEMS MIOPEN_BATCH_NORM FORWARD")
    ...
    def backward(
        ...
        logger.debug("METAX GEMS MIOPEN_BATCH_NORM BACKWARD")
        ...
        weight_grad = torch.empty((input_3d.shape[1],), dtype=input.dtype, device=input.device)
        bias_grad = torch.empty((input_3d.shape[1],), dtype=input.dtype, device=input.device)

        BLOCK_M = 16
        BLOCK_N = 256
        grid = (input_3d.shape[1],)
