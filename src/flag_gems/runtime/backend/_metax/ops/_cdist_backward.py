...
def _cdist_backward_p2_kernel(
    ...
    # Grid: (N, P) - each (i, j) pair processed by one program
    pid_n = tle.program_id(0)
    # pid_p = tle.program_id(1)  # Not used in this kernel

    # Load x1[*, pid_n, :] - the row we're computing gradient for
    x1_offset = pid_n * stride_x1n
    x1_row_ptr = x1_ptr + x1_offset

    # Load all elements of x1[pid_n, :]
...
def _cdist_backward_inf_kernel(
    ...
    for j in range(P):
        ...
        # Only contribute where |diff| == cdist (i.e., this dimension achieves the max)
...
def _cdist_backward_general_kernel(
    ...
    for j in range(P):
        ...
        # For general p:
        # d/dx1 ||x1 - x2||_p = sign(x1 - x2) * |x1 - x2|^(p-1) / ||x1 - x2||_p^(p-1)
        #                      = sign(x1 - x2) * (|x1-x2|/||x1-x2||_p)^(p-1)
...
def _cdist_backward(grad, x1, x2, p, cdist):
    ...
    logger.debug("METAX GEMS _CDIST_BACKWARD")
    ...
    # For now, handle batch=1 case for simplicity
    # TODO: Extend to handle arbitrary batch sizes
    ...
    # Use N grid - each program computes gradient for one row of x1
    grid = (N,)
