...
def heur_block_n(args):
    # Cap BLOCK_N to avoid too large tensor allocation
    # The max BLOCK_N from configs is 1024
    block_n = triton.next_power_of_2(args["N"])
    return min(block_n, 1024)
...
def mean_dim_kernel(
    ...
    mean_val = summed / N
    tl.store(out, mean_val, row_mask)
...
def mean_dim(inp, dim=None, keepdim=False, *, dtype=None):
    logger.debug("METAX GEMS MEAN DIM")
