...
def apply_rotary_pos_emb(
    ...
    """
    logger.debug("METAX GEMS APPLY_ROTARY_POS_EMB")
    assert (
        k.shape[-1] == q.shape[-1]
    ...
    has_pos_ids = position_ids is not None
    if has_pos_ids:
        assert (
    ...
    if inplace:
        ...
        with torch_device_fn.device(q.device):
            apply_rotary_pos_emb_inplace_kernel[grid](
                ...
                cos,
                sin,
                position_ids if has_pos_ids else position_ids,
                q.stride(0),
                q.stride(1),
                q.stride(2),
                k.stride(0),
                k.stride(1),
                k.stride(2),
                position_ids.stride(0) if has_pos_ids else 0,
                cos.stride(0),
                sin.stride(0),
                seq_len,
                has_pos_ids,
