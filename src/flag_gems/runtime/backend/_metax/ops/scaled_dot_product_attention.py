...
# Adapted for Metax backend with tle.program_id() instead of tl.program_id()
@triton.jit
def _attn_fwd_inner(
    acc,
    l_i,
...
# Filter to keep small head dim configs
def keep_small_head_dim(cfg):
    return cfg.kwargs.get("BLOCK_N", 0) <= cfg.kwargs.get("HEAD_DIM", 256)

filtered_configs = list(filter(keep_small_head_dim, configs))
...
def _attn_fwd(
    ...
    # Use tl.program_id for kernel program identification
...
def scaled_dot_product_attention_forward(
    ...
    logger.debug("METAX GEMS SCALED DOT PRODUCT ATTENTION FORWARD")
    # shape constraints
...
class ScaleDotProductAttention(torch.autograd.Function):
    ...
    @staticmethod
    def backward(ctx, do):
        # For simplicity, we don't implement backward in Metax specialization
        # Fall back to torch autograd
        query, key, value, o, M = ctx.saved_tensors
        raise NotImplementedError("Backward pass not implemented in Metax specialization")
