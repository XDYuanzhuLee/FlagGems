...
def rms_norm_loop_kernel(
    ...
    for start_n in range(0, N, TILE_N):
        n_offsets = start_n + tl.arange(0, TILE_N)
        mask = n_offsets < N
        x = tl.load(in_ptr + pid * N + n_offsets, mask=mask, other=0.0).to(cdtype)
...
def rms_norm_forward(x, normalized_shape, weight, eps=1e-5):
    logger.debug("METAX GEMS RMS_NORM FORWARD")
    ...
    with torch_device_fn.device(x.device):
        # For small N (≤256), use BLOCK_SIZE = next_power_of_2(N)
        # This works because 256 * 4 bytes (for float32 array) = 1KB which fits
        if N <= 256:
            ...
            # For N > 256, use the loop kernel with small tile to reduce memory usage
            # The loop kernel processes in chunks, reducing peak memory
            TILE_N = 64
            rms_norm_loop_kernel[M,](y, inv_rms, x, weight, N, eps, TILE_N)
