...
@libentry()
@triton.autotune(configs=runtime.get_tuned_config("logsumexp"), key=["M", "N"])
@triton.jit
def logsumexp_kernel(
    ...
    # Use float32 for accumulation to avoid type mismatch issues with bfloat16
    ...
    # First pass: compute row max for numerical stability
    _all = tl.full([BLOCK_M, BLOCK_N], value=float("-inf"), dtype=acc_type)
    for off in range(0, N, BLOCK_N):
        cols = off + tl.arange(0, BLOCK_N)[None, :]
        col_mask = cols < N
        mask = row_mask and col_mask
        a = tl.load(inp + cols, mask, other=float("-inf")).to(acc_type)
        _all = tl.maximum(_all, a)

    row_max = tl.max(_all, axis=1)[:, None]

    # Second pass: compute sum of exp(x - max) and logsumexp
    acc = tl.full([BLOCK_M, BLOCK_N], value=0.0, dtype=tl.float32)
    for off in range(0, N, BLOCK_N):
        cols = off + tl.arange(0, BLOCK_N)[None, :]
        col_mask = cols < N
        mask = row_mask and col_mask
        ...
        acc = tl.where(mask, acc + exp_val, acc)

    sum_exp = tl.sum(acc, axis=1)[:, None]
...
def logsumexp(inp, dim, keepdim=False):
    logger.debug("METAX GEMS LOGSUMEXP")
