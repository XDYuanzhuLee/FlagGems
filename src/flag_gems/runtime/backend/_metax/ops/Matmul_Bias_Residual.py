...
@libentry()
@libtuner(
    configs=runtime.get_tuned_config("matmul_bias_residual"),
    key=["M", "N", "K"],
)
@triton.heuristics(
    {
...
def matmul_bias_residual(mat1, mat2, bias, residual, *, beta=1.0, alpha=1.0):
    logger.debug("METAX GEMS Matmul_Bias_Residual")
    ...
    logger.debug(
        "METAX GEMS Matmul_Bias_Residual, [shape info]: [-, %s, %s, %s](batch, M, N, K), "
