...
# Use tl_extra_shim functions for cross-backend compatibility
...
def addmm_activation_kernel(
    ...
    if use_gelu:
        ...
        c = (0.5 * x_fp32 * (1 + erf(x_fp32 * scale))).to(bias.dtype)
    else:
        # ReLU activation
        c = tl.where(accumulator > 0, accumulator, 0.0).to(bias.dtype)

    tl.store(c_ptrs, c, mask=c_mask)


def _addmm_activation(self, mat1, mat2, *, beta=1, alpha=1, use_gelu=False):
    logger.debug("METAX GEMS _addmm_activation")
    ...
    logger.debug(
        "METAX GEMS _addmm_activation, [shape info]: [-, %s, %s, %s](batch, M, N, K), "
        "[A column-major]: %s, [B column-major]: %s, [bias column-major]: %s, "
        "[use_gelu]: %s, [alpha]: %s, [beta]: %s",
        M,
        N,
        K,
        mat1.stride(0) == 1,
    ...
    bias = self.broadcast_to(out.shape).contiguous()
