...
def grid_sampler_2d_kernel(
    ...
    if interpolation_mode == 0:  # Bilinear
        ...
        if padding_mode == 0:  # Zeros
            ...
            # Reflect coordinates
            def reflect_coord(coord, max_val):
                coord = tl.minimum(coord, 2 * max_val - 2 - coord)
                coord = tl.maximum(coord, -coord)
                return coord

            x0_ref = reflect_coord(x0, IW)
            y0_ref = reflect_coord(y0, IH)
            x1_ref = reflect_coord(x1, IW)
            y1_ref = reflect_coord(y1, IH)
        ...
        # Load input values
        # input is (N, C, IH, IW), stride: (C*IH*IW, IH*IW, IW, 1)
        input_stride = C * IH * IW

        def load_input_pixel(n_val, c_val, y_val, x_val, valid):
            offset = ((n_val * C + c_val) * IH + y_val) * IW + x_val
            val = tl.load(input_ptr + offset, mask=valid, other=0.0)
            return val

        # Load four corners
        v00 = load_input_pixel(n, c, y0_clamped, x0_clamped, mask & x0_valid & y0_valid)
        v01 = load_input_pixel(n, c, y0_clamped, x1_clamped, mask & x1_valid & y0_valid)
        v10 = load_input_pixel(n, c, y1_clamped, x0_clamped, mask & x0_valid & y1_valid)
        v11 = load_input_pixel(n, c, y1_clamped, x1_clamped, mask & x1_valid & y1_valid)
        ...
        if padding_mode == 0:  # Zeros - need to handle invalid pixels specially
            ...
            # Compute final value
            result = (v00 * wx0 * wy0 + v01 * wx1 * wy0 +
        ...
        if padding_mode == 0:  # Zeros
            ...
            x_nearest = reflect_coord(x_nearest, IW)
            y_nearest = reflect_coord(y_nearest, IH)
            x_nearest = tl.minimum(tl.maximum(x_nearest, 0), IW - 1)
            y_nearest = tl.minimum(tl.maximum(y_nearest, 0), IH - 1)
        ...
        v00 = tl.load(input_ptr + ((n * C + c) * IH + y0_clamped) * IW + x0_clamped, mask=mask)
        v01 = tl.load(input_ptr + ((n * C + c) * IH + y0_clamped) * IW + x1_clamped, mask=mask)
        v10 = tl.load(input_ptr + ((n * C + c) * IH + y1_clamped) * IW + x0_clamped, mask=mask)
        v11 = tl.load(input_ptr + ((n * C + c) * IH + y1_clamped) * IW + x1_clamped, mask=mask)
