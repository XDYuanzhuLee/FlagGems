...
def miopen_batch_norm_backward(
    ...
    """
    Iluvatar specialized implementation of miopen_batch_norm_backward.

    This implements the backward pass of batch normalization using Triton kernels.
    The interface is compatible with aten::miopen_batch_norm_backward.
    """
    logger.debug("ILUVATAR GEMS MIOOPEN_BATCH_NORM_BACKWARD")
    ...
    # Compute inv_std from save_var (save variance) using torch.rsqrt
    # save_mean and save_var are float32 in PyTorch's miopen_batch_norm_backward
    save_invstd = torch.rsqrt(save_var.to(torch.float32) + epsilon)
