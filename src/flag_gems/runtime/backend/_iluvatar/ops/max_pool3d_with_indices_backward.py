...
@triton.autotune(
    configs=[
        triton.Config({"BLOCK_D": 8, "BLOCK_H": 8, "BLOCK_W": 8}, num_warps=4, num_stages=2),
        triton.Config({"BLOCK_D": 8, "BLOCK_H": 8, "BLOCK_W": 16}, num_warps=4, num_stages=2),
        triton.Config({"BLOCK_D": 8, "BLOCK_H": 16, "BLOCK_W": 8}, num_warps=4, num_stages=2),
        triton.Config({"BLOCK_D": 8, "BLOCK_H": 16, "BLOCK_W": 16}, num_warps=4, num_stages=2),
        triton.Config({"BLOCK_D": 16, "BLOCK_H": 8, "BLOCK_W": 8}, num_warps=4, num_stages=2),
        triton.Config({"BLOCK_D": 16, "BLOCK_H": 8, "BLOCK_W": 16}, num_warps=4, num_stages=2),
        triton.Config({"BLOCK_D": 16, "BLOCK_H": 16, "BLOCK_W": 8}, num_warps=4, num_stages=2),
        triton.Config({"BLOCK_D": 16, "BLOCK_H": 16, "BLOCK_W": 16}, num_warps=8, num_stages=2),
    ],
...
def max_pool3d_forward_kernel(
    ...
    out_mask = (out_d_offsets[:, None, None] < out_d) & (out_h_offsets[None, :, None] < out_h) & (out_w_offsets[None, None, :] < out_w)
...
@triton.autotune(
    configs=[
        ...
        triton.Config({"BLOCK_IN_D": 8, "BLOCK_IN_H": 16, "BLOCK_IN_W": 16}, num_warps=4),
        triton.Config({"BLOCK_IN_D": 16, "BLOCK_IN_H": 8, "BLOCK_IN_W": 8}, num_warps=4),
        triton.Config({"BLOCK_IN_D": 16, "BLOCK_IN_H": 8, "BLOCK_IN_W": 16}, num_warps=4),
        triton.Config({"BLOCK_IN_D": 16, "BLOCK_IN_H": 16, "BLOCK_IN_W": 8}, num_warps=4),
        triton.Config({"BLOCK_IN_D": 16, "BLOCK_IN_H": 16, "BLOCK_IN_W": 16}, num_warps=8),
    ],
...
def max_pool3d_backward_kernel(
    ...
    num_hw_blocks = tl.cdiv(in_h, BLOCK_IN_H) * tl.cdiv(in_w, BLOCK_IN_W)
    d_block_idx = pid_dhw // num_hw_blocks
    hw_block_idx = pid_dhw % num_hw_blocks
    h_block_idx = hw_block_idx // tl.cdiv(in_w, BLOCK_IN_W)
    w_block_idx = hw_block_idx % tl.cdiv(in_w, BLOCK_IN_W)
    ...
    for kd in tl.static_range(0, kernel_d):
        for kh in tl.static_range(0, kernel_h):
            for kw in tl.static_range(0, kernel_w):
                ...
                out_offsets = safe_d_out * out_stride_d + safe_h_out * out_stride_h + safe_w_out * out_stride_w
...
def max_pool3d_with_indices(
    ...
    logger.debug("ILUVATAR GEMS MAX_POOL3D_WITH_INDICES FORWARD")
    ...
    out_d = max_pool3d_output_size(in_d, kernel_d, stride_d, padding_d, dilation_d, ceil_mode)
    out_h = max_pool3d_output_size(in_h, kernel_h, stride_h, padding_h, dilation_h, ceil_mode)
    out_w = max_pool3d_output_size(in_w, kernel_w, stride_w, padding_w, dilation_w, ceil_mode)
...
def max_pool3d_with_indices_backward(
    ...
    logger.debug("ILUVATAR GEMS MAX_POOL3D_WITH_INDICES_BACKWARD")
    ...
    grid = lambda meta: (
        in_n * in_c,
        triton.cdiv(in_d, meta["BLOCK_IN_D"]) * triton.cdiv(in_h, meta["BLOCK_IN_H"]) * triton.cdiv(in_w, meta["BLOCK_IN_W"]),
    )
