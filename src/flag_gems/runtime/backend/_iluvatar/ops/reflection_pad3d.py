...
@triton.jit
def copy_tensor_kernel_3d(in_ptr, out_ptr, B, D, H, W, BLOCK_DHW: tl.constexpr):
    pid_b = tl.program_id(axis=0)
    pid_n = tl.program_id(axis=1)
...
def launch_reflection_pad3d(input: torch.Tensor, padding, out: torch.Tensor = None):
    ...
    pad_left, pad_right, pad_top, pad_bottom, pad_front, pad_back = [int(p) for p in padding]

    # Validate padding values
    if pad_left < 0 or pad_right < 0 or pad_top < 0 or pad_bottom < 0 or pad_front < 0 or pad_back < 0:
    ...
    if pad_left >= W_in or pad_right >= W_in or pad_top >= H_in or pad_bottom >= H_in or pad_front >= D_in or pad_back >= D_in:
        raise ValueError(
    ...
    if pad_left == 0 and pad_right == 0 and pad_top == 0 and pad_bottom == 0 and pad_front == 0 and pad_back == 0:
        BLOCK_DHW = 256
        grid = (B, triton.cdiv(D_in * H_in * W_in, BLOCK_DHW))
        copy_tensor_kernel_3d[grid](x, out, B, D_in, H_in, W_in, BLOCK_DHW=BLOCK_DHW)
        return out

    BLOCK_DHW = 256
    grid = (B, triton.cdiv(D_out * H_out * W_out, BLOCK_DHW))
    reflection_pad3d_kernel[grid](
        x, out, B, D_in, H_in, W_in, pad_left, pad_top, pad_front, D_out, H_out, W_out, BLOCK_DHW=BLOCK_DHW
    )
    return out


def reflection_pad3d(input: torch.Tensor, padding):
    logger.debug("ILUVATAR GEMS REFLECTION_PAD3D")
...
def reflection_pad3d_out(input: torch.Tensor, padding, out: torch.Tensor):
    logger.debug("ILUVATAR GEMS REFLECTION_PAD3D_OUT")
