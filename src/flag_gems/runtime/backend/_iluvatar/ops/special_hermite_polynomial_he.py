...
def special_hermite_polynomial_he_func(x, n):
    ...
    x5 = x4 * x_fp  # noqa: F841
    x6 = x4 * x2
    x7 = x6 * x_fp  # noqa: F841
    x8 = x6 * x2
    result = tl.where(
        n_int == 9,
        x8 * x_fp - 36.0 * x6 * x_fp + 378.0 * x4 * x_fp - 1260.0 * x3 + 945.0 * x_fp,
        result,
    )
...
def special_hermite_polynomial_he(x, n):
    ...
    if isinstance(x, torch.Tensor) and isinstance(n, torch.Tensor):
        ...
        x_dtype = torch.float32 if isinstance(x, (int, float)) else x.dtype
        result = torch.special.hermite_polynomial_he(torch.tensor(x, dtype=x_dtype), n)
