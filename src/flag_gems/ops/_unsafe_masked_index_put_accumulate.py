...
def _unsafe_masked_index_put_accumulate(inp, mask, indices, values):
    ...
    # Validate shapes
    assert (
        mask.shape == indices_tensor.shape
    ), f"mask and indices must have same shape, got {mask.shape} and {indices_tensor.shape}"
    assert (
        mask.shape == values.shape
    ), f"mask and values must have same shape, got {mask.shape} and {values.shape}"
