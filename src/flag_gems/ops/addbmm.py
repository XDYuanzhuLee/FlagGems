...
@libtuner(
    ...
    if __import__("os").environ.get("USE_FLAGTUNE") == "1"
...
def addbmm_kernel(
    ...
    input_ptrs = input_tensor + offs_m[:, None] * stride_input_m + offs_n[None, :] * stride_input_n
    output_ptrs = (
...
def addbmm(input, batch1, batch2, beta=1.0, alpha=1.0):
    ...
    assert input.shape == (M, N), f"input shape mismatch: expected ({M}, {N}), got {input.shape}"
