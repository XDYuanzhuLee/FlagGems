...
def test_accuracy_bitwise_right_shift_(shape, dtype):
    ...
    inp2 = torch.randint(
        low=0, high=16, size=shape, dtype=dtype, device="cpu"
    ).to(flag_gems.device)
    ref_inp1 = to_reference(inp1)
    ref_inp2 = to_reference(inp2)

    ref_out = torch.bitwise_right_shift(ref_inp1, ref_inp2)
    with flag_gems.use_gems():
...
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("dtype", INT_DTYPES)
def test_accuracy_bitwise_right_shift__inplace(shape, dtype):
    inp1 = torch.randint(
        low=0, high=0x7FFF, size=shape, dtype=dtype, device="cpu"
    ).to(flag_gems.device)
    inp2 = torch.randint(
