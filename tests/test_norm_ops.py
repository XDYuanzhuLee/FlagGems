...
@pytest.mark._batch_norm_no_update
@pytest.mark.parametrize(
    "shape",
    [
        (16, 3),
...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
@pytest.mark.parametrize("affine", [True, False])
def test_accuracy__batch_norm_no_update(shape, dtype, affine):
    if flag_gems.vendor_name == "cambricon":
        torch.manual_seed(23)
        torch.mlu.manual_seed_all(23)
    C = shape[1]
    ...
    ref_out, ref_mean, ref_var, ref_reserved = torch.ops.aten._batch_norm_no_update(
        ref_inp,
        ref_weight,
        ref_bias,
        ref_running_mean,
    ...
    with flag_gems.use_gems():
        res_out, res_mean, res_var, res_reserved = torch.ops.aten._batch_norm_no_update(
            inp,
            weight,
            bias,
            running_mean,
    ...
    # Verify running stats are NOT updated
    gems_assert_close(running_mean, ref_running_mean, dtype)
    gems_assert_close(running_var, ref_running_var, dtype)
