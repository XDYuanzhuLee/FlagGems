...
def test_accuracy_special_chebyshev_polynomial_v(shape, dtype):
    # Test with tensor n (element-wise)
    x = torch.randn(shape, dtype=dtype, device=flag_gems.device)
...
@pytest.mark.parametrize("shape", SPECIAL_SHAPES)
@pytest.mark.parametrize("dtype", [torch.float32])
def test_accuracy_special_chebyshev_polynomial_v_scalar_n(shape, dtype):
    # Test with scalar n
    x = torch.randn(shape, dtype=dtype, device=flag_gems.device)
    n = 3  # scalar
    ...
    ref_out = torch.special.chebyshev_polynomial_v(ref_x, n)
    with flag_gems.use_gems():
        res_out = torch.special.chebyshev_polynomial_v(x, n)
