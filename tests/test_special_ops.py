...
# Reference implementation for _dyn_quant_pack_4bit_weight for testing
...
def test_accuracy_dyn_quant_pack_4bit_weight(out_features, in_features, block_size):
    ...
    weights = torch.randn(out_features, in_features, dtype=dtype, device=flag_gems.device)

    num_groups = (in_features + block_size - 1) // block_size
    scales_zeros = torch.randn(out_features, num_groups * 2, dtype=dtype, device=flag_gems.device)
