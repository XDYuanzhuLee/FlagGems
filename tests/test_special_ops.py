...
# Test shapes for replication_pad3d_backward
...
@pytest.mark.parametrize("padding", REPLICATION_PAD3D_PADDING)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_replication_pad3d_backward(shape, padding, dtype):
    # Create input tensor
    inp = torch.randn(shape, dtype=dtype, device=flag_gems.device)

    # Compute output shape after padding
    N, C, D, H, W = shape
    pad_w_before, pad_w_after, pad_h_before, pad_h_after, pad_d_before, pad_d_after = padding
    ...
    grad_output = torch.randn(N, C, D_out, H_out, W_out, dtype=dtype, device=flag_gems.device)
    ...
    # Use more lenient tolerance for float16 and bfloat16 due to atomic_add precision
    # The atomic_add operation causes precision loss when accumulating gradients
    if dtype in (torch.float16, torch.bfloat16):
        gems_assert_close(res_out, ref_out, dtype, atol=0.05)
    else:
        gems_assert_close(res_out, ref_out, dtype)
