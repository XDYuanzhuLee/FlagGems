...
# Tests for embeddingSpMDMGrad operator
@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA is not available")
@pytest.mark.skipif(TO_CPU, reason="Unsupported in CPU mode")
...
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_embeddingSpMDMGrad(EmbeddingSize, Batch, M, N, dtype):
    """Test accuracy for embeddingSpMDMGrad backward pass."""
    if flag_gems.vendor_name == "kunlunxin":
        torch.manual_seed(0)
        torch.cuda.manual_seed_all(0)
    ...
    with flag_gems.use_gems():
        res_in_grad = embedding_spdm_grad(
...
@pytest.mark.parametrize("N", [8] if TO_CPU else [128, 256])
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
def test_accuracy_embeddingSpMDMGrad_scale_by_freq(EmbeddingSize, Batch, M, N, dtype):
    """Test accuracy for embeddingSpMDMGrad with scale_grad_by_freq."""
    if flag_gems.vendor_name == "kunlunxin":
        torch.manual_seed(0)
        torch.cuda.manual_seed_all(0)
    ...
    # Reference implementation with scale_grad_by_freq
    ref_in_grad = torch.ops.aten.embedding_backward(
        ref_grad, ref_indices, num_weights, -1, True, False
    )
    ...
    with flag_gems.use_gems():
        res_in_grad = embedding_spdm_grad(
            res_grad, res_indices, num_weights, -1, True, False
        )
