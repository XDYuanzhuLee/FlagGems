...
# Benchmark for KV_Cache_Update metax specialized operator
def torch_KV_Cache_Update_ref(
    key,  # [num_tokens, num_heads, head_size]
    value,  # [num_tokens, num_heads, head_size]
    key_cache,  # [num_blocks, num_heads, head_size/x, block_size, x]
    value_cache,  # [num_blocks, num_heads, head_size, block_size]
    ...
    num_tokens = slot_mapping.numel()
    block_size = key_cache.size(3)
    reshaped_key = key.reshape(num_tokens, *key_cache[0, :, :, 0, :].shape) * k_scale
    for i in range(num_tokens):
        slot = slot_mapping[i].item()
        block_idx = slot // block_size
        block_offset = slot % block_size
...
class KVCacheUpdateBenchmark(GenericBenchmark):
    """
    benchmark for KV_Cache_Update metax specialized operator
    """
...
def test_perf_KV_Cache_Update():
    ...
    bench = KVCacheUpdateBenchmark(
        op_name="KV_Cache_Update",
        input_fn=input_kwargs,
        torch_op=torch_KV_Cache_Update_ref,
        dtypes=FLOAT_DTYPES,
    )
    # Use metax specialized version
    from flag_gems.runtime.backend._metax.ops import KV_Cache_Update as metax_kv
    bench.set_gems(metax_kv.kv_cache_update)
    bench.run()
